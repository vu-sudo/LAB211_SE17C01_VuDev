...
public class InputLibrary {
    ...
    public String inputUserName(String msg, ArrayList<User> userList) {
        ...
        do {
            ...
            } else if (!validator.validatePattern(resultString, "^(?=.*[a-z])(?=.*[A-Z])(?!.*\\s).+$") || resultString.length() < 5) {
                System.out.println("You must enter least at 5 character, and no space, one character is upper!");
            }
        } while(isDuplicated == true || resultString.length() < 5 || !validator.validatePattern(resultString, "^(?=.*[a-z])(?=.*[A-Z])(?!.*\\s).+$"));
        return resultString;
    }
    //input password;
    ...
    public User inputLogUserName(String msg, ArrayList<User> userList) {
        ...
        do {
            ...
            if (!validator.validatePattern(findString, "^(?=.*[a-z])(?=.*[A-Z])(?!.*\\s).+$") ||    findString.length() < 5) {
                System.out.println("You must enter least at 5 character, and no space, one character is upper!");
            } else {
        ...
        } while(findString.length() < 5 || !validator.validatePattern(findString, "^(?=.*[a-z])(?=.*[A-Z])(?!.*\\s).+$") || checkingUser == null);
        return checkingUser;
    };
}
